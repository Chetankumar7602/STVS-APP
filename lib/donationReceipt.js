...
function buildTextStream(lines) {
  ...
  const textOps = [
    ...
    buildTextBlock({
      ...
      lines: [lines.receiptNo, lines.date, lines.paymentRef],
      leading: 16,
    }),
    buildTextBlock({
      x: 44,
...
export function buildDonationReceiptPdf(donation, options = {}) {
  ...
  const methodLabel = donation.paymentMethod === 'bank_transfer' ? 'Bank Transfer' : 'UPI QR';
  const paymentStatusLabel = isManualQr
    ? donation.manualReviewStatus === 'wrong_amount'
      ? `${methodLabel} Review: Wrong amount selected`
      : donation.manualReviewStatus === 'successful'
        ? `${methodLabel} Review: Successful`
        : donation.manualReviewStatus === 'unsuccessful'
          ? `${methodLabel} Review: Unsuccessful`
          : `${methodLabel} Review: Pending`
  ...
  const textLines = {
    ...
    paymentRef: `UPI/Transaction Ref: ${donation.upiReference || 'N/A'}`,
      ...
      <div style="font-family:Arial,sans-serif;max-width:640px;margin:auto;padding:24px;background:#f8fafc;color:#0f172a;">
        ...
        <div style="background:#ffffff;padding:24px;border:1px solid #dbe4f0;border-top:none;border-radius:0 0 18px 18px;">
          ...
          <table style="width:100%;border-collapse:collapse;margin:18px 0;background:#ffffff;">
            ...
            ${extraRows}
            <tr><td style="padding:10px;border:1px solid #e2e8f0;"><strong>UPI / Transaction Ref</strong></td><td style="padding:10px;border:1px solid #e2e8f0;">${donation.upiReference || 'N/A'}</td></tr>
